...
{
    public class ManagerAuthAttribute : ActionFilterAttribute
    {
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            try
            {
                ...
                if (roleClaim == null || roleClaim.Value.ToLower() != "manager")
                {
                    // 3. Có đăng nhập nhưng KHÔNG PHẢI MANAGER -> Chuyển về trang chủ hoặc trang lỗi
                    context.Result = new RedirectToActionResult("Index", "Home", new { area = "" });
                    return;
                }

                // Nếu là Manager -> Cho phép tiếp tục vào Action
            }
            catch
            {
